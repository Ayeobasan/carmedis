...
const solutionNavs = [
  ...
  },
  {
    ...
    navs: [
      {
        title: "How car works",
        desc: "Get to know how car works properly",
        path: "/how-it-works",
        icon: <HiMiniClock className="size-6" />,
      },
      // {
...
const productNavs = [
  {
    ...
    navs: [
      ...
      // {
      //   title: "ScanPro",
      //   desc: "Discover trusted mechanics using our app",
      //   path: "",
      //   icon: <HiMiniClock className="size-6" />,
      // },
    ],
...
const Navbar = () => {
  ...
  const navigation = [
    ...
    { title: "Car Onwner", path: "https://www.carmedis.com/car-owner/", isDropdown: false, navs: opportunityNavs },
    { title: "Support", path: "https://tawk.to/Carmedis", isDropdown: false, navs: resourcesNavs },
  ];
  ...
  return (
    <>
      ...
      >
        <div className="items-center gap-8 xd:gap-x-14 md:px-8 xl:px-0 px-4 max-w-[1200px] border-b mx-auto md:flex align-middle ">
          <div className="flex items-center justify-between py-3 md:py-6 md:block">
          <Link href="/">
              <Image src={Logo} width={75} height={60} alt="Logo" />
            </Link>
          ...
          >
            <ul className="items-center space-y-6 md:flex md:space-x-3 xd:space-x-6 md:space-y-0">
              {navigation.map((item, idx) => {
                return (
                  <li key={idx}>
                    {item.isDropdown ? (
                      <button
                        className="w-full flex items-center text-sm xd:text-base justify-between gap-1 text-black hover:font-medium hover:text-carmedis"
                        onClick={() =>
                          setDropdownState({
                            idx,
                            isActive: !dropdownState.isActive,
                    ...
                    dropdownState.idx == idx &&
                    dropdownState.isActive ? (
                      <div className="mt-6 inset-x-0 top-[7rem] w-full md:absolute md:border-y md:shadow-md md:-mt-1">
                        <ul className="max-w-[1200px] bg-white mx-auto grid items-center gap-6 md:p-4 xl:px-0 md:grid-cols-2 lg:grid-cols-3">
                          {item?.navs?.map((dropdownItem, idx) => (
                            <li key={idx}>
                              <p className="text-carmedis text-sm">
                                {dropdownItem.label}
             ...
             <div className="flex-1 items-center justify-end gap-x-6 space-y-3 md:flex md:space-y-0">
                ...
                <li>
                  <Link
                    href=""
                    className="block py-3 px-4 font-medium text-xs xd:text-base text-center text-white bg-carmedis hover:bg-carmedis active:bg-carmedis active:shadow-none rounded-2xl shadow md:inline active:scale-95 duration-150"
                  >
                    Request for Car Repair Expert
                  </Link>
                </li>
